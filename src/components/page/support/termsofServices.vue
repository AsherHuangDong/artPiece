  ...
  <div id="support" class="content">
    <div class="box">
      <Breadcrumb separator="<b style='color:#32325d;'>></b>">
        ...
        <BreadcrumbItem style="color:#32325d;">{{ $t('support.TermsofServices') }}</BreadcrumbItem>
      </Breadcrumb>
      <div class="con">
        <div class="title">{{ $t('support.TermsofServices') }}</div>
        <div class="detail">
          <h3>{{ $t('support.TermsofServices') }}</h3>
          <p id="ppData1"></p>
  ...
  methods: {
    getOtherContent() {
      ...
      this.$post('/api/otherContent/getOtherContent', form).then(res => {
        ...
        let ppData1 = document.getElementById('ppData1')
